...
const Agerez = {
  ...
  reTypingPage: function (
    arrayWithTextsLocations,
  ...
  randomArrayElements: {
    ...
    everyOpen: function (array, arrayId) {
      if (
        sessionStorage.getItem(arrayId) !== "" &&
        sessionStorage.getItem(arrayId) !== null &&
        sessionStorage.getItem(arrayId) !== undefined &&
        sessionStorage.getItem(arrayId) !== NaN
      ) {
    ...
    oneTime: function (array, arrayId) {
      if (
        localStorage.getItem(arrayId) !== "" &&
        localStorage.getItem(arrayId) !== null &&
        localStorage.getItem(arrayId) !== undefined &&
        localStorage.getItem(arrayId) !== NaN
      ) {
  ...
  errorMessage: function (message, idOfelementBeforeErrorMessage, fontSize) {
    ...
    document
      .getElementsByTagName(idOfelementBeforeErrorMessage)
      .after(errorMessage);
  },

  tillNewYearCounter: function (
    secondsId,
    minutesId,
    hoursId,
    daysId,
    ...
    let counter = setInterval(() => {
      ...
      let hoursTime = Math.floor(
        (deltaDate % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
      );
      let minutesTime = Math.floor(
        (deltaDate % (1000 * 60 * 60)) / (1000 * 60)
      );
  ...
  countTimeSpentAtSite: function (
    ...
    if (
      localStorage.minutes != null &&
      localStorage.minutes != "" &&
      localStorage.minutes != NaN
    ) {
    ...
    if (
      localStorage.hours != null &&
      localStorage.hours != "" &&
      localStorage.hours != NaN
    ) {
    ...
    if (
      localStorage.days != null &&
      localStorage.days != "" &&
      localStorage.days != NaN
    ) {
    ...
    if (
      localStorage.years != null &&
      localStorage.years != "" &&
      localStorage.years != NaN
    ) {
  ...
  generatePassword: function (lettersCount) {
    let passwordfinal = "";
    let lettersarray =
      "1234567890qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM1234567890";
  ...
  addInputLettersCounter: function (
    maxlengthOfLetters,
    barColor,
    barBackgroundColor,
    haveNumbersCounter,
    locationOfNumbersCounter
  ) {
    /*  in this function 
        you have to put the input in a div and set the div width = the input width you want. 
        and set the input width = (100%) of the div's width */
  ...
  abbText: function (text, maxLength, showThatWasRest) {
    ...
    if (+maxLength === 0) {
      ...
      console.error(
        `Agere : "maxLength" must be number not ${typeof maxLength}`
      );
      ...
      return text?.slice(0, Math.round(maxLength)) + (STWR ? "..." : "");
    }
  },
